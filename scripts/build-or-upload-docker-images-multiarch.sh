...
if [[ "$BRANCH" == "master" || $BRANCH =~ ^v[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
  ...
  IMAGE_TAGS="${IMAGE_TAGS}" PUSHTAG="type=image, push=true" make multiarch-docker-images-only
else
  echo 'skip multiarch docker images upload, only allowed for tagged releases or master (latest tag)'
  IMAGE_TAGS="${IMAGE_TAGS}" PUSHTAG="type=image, push=false" make multiarch-docker-images-only
fi
